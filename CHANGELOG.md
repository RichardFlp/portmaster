...
- `portmaster kill` now hints to use `sudo` (or an Administrator terminal on
  Windows) when the owning process cannot be identified, for example when
  killing a port owned by a system service.
